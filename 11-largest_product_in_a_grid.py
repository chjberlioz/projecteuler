...
for i in range(17):
	for j in range(17):
		...
		if pro < temp1: pro = temp1
		temp2 = a[i][j]*a[i][j+1]*a[i][j+2]*a[i][j+3]
		if pro < temp2: pro = temp2
		temp3 = a[i][j]*a[i+1][j+1]*a[i+2][j+2]*a[i+3][j+3]
		if pro < temp3: pro = temp3

k = 17
while k < 20:
	for i in range(17):
		temp1 = a[i][k]*a[i+1][k]*a[i+2][k]*a[i+3][k]
		if pro < temp1: pro = temp1
		temp2 = a[k][i]*a[k][i+1]*a[k][i+2]*a[k][i+3]
		if pro < temp2: pro = temp2
	k += 1
...
while m >= 3:
	n = 0
	while n <= 16:
		temp = a[n][m]*a[n+1][m-1]*a[n+2][m-2]*a[n+3][m-3]
